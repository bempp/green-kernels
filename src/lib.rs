...
#[allow(dead_code)]
impl<T: RealScalar, S: Simd> SimdFor<T, S> {
    #[inline]
...
trait ComplexScalar:
    RlstScalar<Complex = Self, Real: RealScalar> + Pod + Send + Sync + 'static
{
}

impl ComplexScalar for rlst::c32 {}
impl ComplexScalar for rlst::c64 {}

/// [`rlst::RlstScalar`] extension trait for SIMD operations.
#[allow(dead_code)]
trait RealScalar: RlstScalar<Real = Self> + Pod + Send + Sync + 'static {
...
mod tests {
    ...
    fn test_sin_cos() {
        ...
        for _ in 0..1000 {
            ...
            assert!((s - s_target).abs() <= 1e-5);
            assert!((c - c_target).abs() <= 1e-5);
        }
        ...
        for _ in 0..1000 {
            ...
            let (s_target, c_target) = x.sin_cos();
            assert!((s - s_target).abs() <= 1e-12);
            assert!((c - c_target).abs() <= 1e-12);
        }
    }
}
